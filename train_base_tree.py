...
def main():
  ...
  for feature in features:
    ...
    for i in range(4):
      ...
      if not exists(f'results/btree/cms/{feature}'):
        makedirs(f'results/btree/cms/{feature}')
      cm.plot(cmap=plt.cm.Blues).figure_.savefig(f'results/btree/cms/{feature}/fold_{i}.png')
      plt.close()
    ...
    cm = ConfusionMatrixDisplay(confusion_matrix=conf_mat, display_labels=['Low', 'Medium', 'High'])
    cm.plot(cmap=plt.cm.Purples).figure_.savefig(f'results/btree/cms/{feature}/confusion_matrix.png')
  ...
  with open('./results/btree/groups.json', 'w+') as file:
    json.dump(group_map, file)

  with open('./results/btree/metrics.json', 'w') as file:
    json.dump(feature_map, file)
      
  fig = plt.figure(figsize=(35, 45))
  _ = sklearn.tree.plot_tree(clf, 
                      feature_names=list(training_data.iloc[:,1:-1].columns),  
                      class_names=['Low', 'Medium', 'High'],
                      filled=True)
  fig.savefig("results/btree/btree.png")
