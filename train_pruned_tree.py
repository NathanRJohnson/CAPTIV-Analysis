...
from pruned_dtree import prune_tree, predict_points
...
def main():
  ...
  for feature in features:
    ...
    for i in range(4):
      ...
      y_pred = predict_points(tree, pruned_tree, x_test, y_test)
      ...
      if not exists(f'results/ptree/cms/{feature}'):
        makedirs(f'results/ptree/cms/{feature}')
      cm.plot(cmap=plt.cm.Blues).figure_.savefig(f'results/ptree/cms/{feature}/fold_{i}.png')
      plt.close()
    ...
    cm = ConfusionMatrixDisplay(confusion_matrix=conf_mat, display_labels=['Low', 'Medium', 'High'])
    cm.plot(cmap=plt.cm.Purples).figure_.savefig(f'results/ptree/cms/{feature}/confusion_matrix.png')

  # print final values
  conf_mat = confusion_matrix(all_true, all_pred, labels=['Low', 'Medium', 'High'])
  cm = ConfusionMatrixDisplay(confusion_matrix=conf_mat, display_labels=['Low', 'Medium', 'High'])
  cm.plot(cmap=plt.cm.Purples).figure_.savefig(f'results/ptree/cms/confusion_matrix.png')  
  ...
  with open('./results/ptree/groups.json', 'w+') as file:
    json.dump(group_map, file)

  with open('./results/ptree/metrics.json', 'w') as file:
